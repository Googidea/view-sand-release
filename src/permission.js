    ...
    if (to.path === '/login') {
      ...
      if (store.getters.roles.length === 0) { // 判断当前用户是否已拉取完user_info信息
        store.dispatch('GetUserInfo').then(() => { // 拉取user_info
          const roles = store.getters.resources||store.getters.roles // note: roles must be a array! such as: ['editor','develop']
        ...
        if (hasPermission(store.getters.resources||store.getters.roles, to.meta.roles||to.meta.resources)) {
          showMap(to);
          next()
        } else {
...
router.beforeEach((to, from, next) => {
  ...
  if(store.getters.security_query !== true&&to.path!="/login"){
